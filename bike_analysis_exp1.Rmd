...
How many riders (max) in any hour interval?

```{r}
dfrh = dfr1 %>% group_by(wday(t,label=T),hour(t)) %>% 
  summarise(max_riders = max(riders),ave_riders = mean(riders))
```



Which stations "gain" bikes over time and which ones "lose" bikes?

For gains and losses I will:
  *filter to make sure that the station_id is not "NA"
  *group by the station_id
  *count the number of starts/ends per station

```{r}
gains = dfb %>% filter(start_station_id > 0) %>%
  group_by(station_id = start_station_id) %>% 
  summarise(starts = n())


losses = dfb %>% filter(end_station_id > 0) %>%
  group_by(station_id = end_station_id) %>%
  summarise(ends = n())

```

Let's join these two dataframes and compute net gains

```{r}
net = gains %>% full_join(losses) %>%
  replace_na(list(starts = 0)) %>%
  group_by(station_id) %>%
  mutate(net_gains = starts - ends)
```

Visualize these results to see how the net gains of each station compare to one another 

```{r}
net %>% ggplot(aes(x=net_gains)) + 
  geom_histogram(bins = 500)
```

Here we can see that a majority of the stations have somewhere near a net gain of 0. It is also worth noting that the count of net gains per station seems to be normally distributed. 