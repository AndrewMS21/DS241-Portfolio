...
ncol(dn)
```
There are 1643 rows and 6 columns. Each row represents a certain Denny's and the variables are components of each location.
...
ncol(lq)
```
There are 909 rows and 6 columns. Each row represents a certain La Quinta and the variables are components of each location.
...
There are La Quinta's located in Canada, Mexico, Honduras, Turkey, Chile, and Columbia. According to the linked website there are no Denny's recorded outsied of the US.
...
Check to see if there is some abbreviation other than a US state as the state variable for a location.
...
lq = lq %>%
  mutate(country = case_when(
    ...
    state == "ANT"           ~ "Colombia",
    state %in% c("AG", "CH", "NL", "VE", "PU", "SL", "QR") ~ "Mexico",
    state == "FM"            ~ "Honduras",
...
I'm not surprised that California has the most Denny's but the amount that they have surprises me. The same goes for Texas with La Quinta. 
...
```{r}
ggplot(dn_lq_NC, mapping = aes(x = longitude, y = latitude, color = establishment, alpha = 0.2)) +
  geom_point()
```
...
```{r}
ggplot(dn_lq_TX, mapping = aes(x = longitude, y = latitude, color = establishment, alpha = 0.1)) +
  geom_point()
```
