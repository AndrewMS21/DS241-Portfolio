...
Try it on Alaska
```{r}
dnlqak_mindist = analyze_state_q("AK")
```

Now on Texas
```{r}
dnlqtx_mindist = analyze_state_q("TX")
```

## Further analysis using other fast food restaurants

Import data for locations of other fast food restaurants across the US
```{r}
FastFoodRestaurants <- read_excel("~/DS241/FastFoodRestaurants.xls")
```
...
```{r}
analyze_all_food_q = function (this_state) {
  ...
  ff_state = FastFoodRestaurants %>%
    rename(state = province) %>%
    filter(state == this_state) %>%
    sample_n(size = nrow(dn_state))
...
ff_lq_state_mindist <- full_join(lq_state, ff_state, by = "state") %>%
    mutate(distance=
           haversine(longitude.x, latitude.x, longitude.y, latitude.y, round = 4)) %>%
  group_by(address.x) %>%
    summarise(closest = min(distance))  

all_mindist <- full_join(dn_lq_state_mindist, ff_lq_state_mindist, by = "address.x") %>%
  rename(closest_dennys = closest.x) %>%
  rename(closest_ff = closest.y)
...
Try it on Alaska
```{r}
all_ak_mindist = analyze_all_food_q("AK")

all_ak_mindist %>% ggplot() +
  geom_histogram(aes(x=closest_dennys), colour = 'blue', alpha = 0.2) +
  geom_histogram(aes(x=closest_ff), colour = 'red', alpha = 0.2) +
  xlab("closest")
```
It is important to note that blue represents Denny's and red represents all other average restaurants.

Now on Texas
```{r}
all_tx_mindist = analyze_all_food_q("TX")
all_tx_mindist %>% ggplot() +
  geom_histogram(aes(x=closest_dennys), colour = 'blue', alpha = 0.2) +
  geom_histogram(aes(x=closest_ff), colour = 'red', alpha = 0.2) +
  xlab("closest")
```
...
Now on Maryland
```{r}
all_md_mindist = analyze_all_food_q("MD")
all_md_mindist %>% ggplot() +
  geom_histogram(aes(x=closest_dennys), colour = 'blue', alpha = 0.2) +
  geom_histogram(aes(x=closest_ff), colour = 'red', alpha = 0.2) +
  xlab("closest")
```
...
Now on New York
```{r}
all_ny_mindist = analyze_all_food_q("NY")
all_ny_mindist %>% ggplot() +
  geom_histogram(aes(x=closest_dennys), colour = 'blue', alpha = 0.2) +
  geom_histogram(aes(x=closest_ff), colour = 'red', alpha = 0.2) +
  xlab("closest")
```

After taking a look at these histograms for a few different states, there does not seem to be a decisive answer to the question of "are Denny's closer to La Quinta's then the average restaurant?" If you run the function for each state a few times, you can see that the distance to Denny's is fairly similar to that of an average restaurant. 
